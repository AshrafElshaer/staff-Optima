...
function generateEventDuration(): number {
  ...
  return durations[randomIndex] ?? 30 // Fallback to 30 minutes if undefined
}
...
export function generateMockEvents(): CalendarEvent[] {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    events.push({
      id: `event-${i + 1}`,
      // @ts-expect-error
      title: EVENT_TITLES[Math.floor(Math.random() * EVENT_TITLES.length)] || EVENT_TITLES[0],
      // @ts-expect-error
      color: EVENT_COLORS[Math.floor(Math.random() * EVENT_COLORS.length)] || EVENT_COLORS[0],
      start: startTime,
      end: endTime,
    })
  }
